...
// 收集指标数据
// collectMetrics 从Prometheus收集指标数据
// 参数:
// - client: Prometheus API客户端,用于查询指标数据
// - config: 应用配置,包含Prometheus URL和指标查询配置
// 返回:
// - *report.ReportData: 包含所有收集到的指标数据的报告结构体
// - error: 如果收集过程中出现错误则返回error,否则返回nil
func collectMetrics(client v1.API, config *Config) (*report.ReportData, error) { // 收集指标数据
	...
	// ReportData在pkg/report/generator.go中定义
	// 创建报告数据结构体,包含以下字段:
	// - Timestamp: 报告生成的时间戳
	// - MetricGroups: 按指标类型分组的监控数据,key是指标类型(如resource/kubernetes),value是该类型下的所有指标
	// - ChartData: 用于生成图表的数据,key是图表标识,value是图表数据的JSON字符串
	// MetricGroups使用map是因为需要通过指标类型(如resource/kubernetes)快速查找对应的指标组
	// ChartData使用map是因为需要通过图表标识快速查找对应的图表数据
	...
	for _, metricType := range config.MetricTypes { // 遍历每个指标类型
		...
		for _, metric := range metricType.Metrics { // 遍历每个指标
			...
			case model.Vector: // 处理向量类型的查询结果
				// model.Vector 是 Prometheus 查询结果的一种类型,表示某个时间点的一组样本数据
				// 每个样本包含指标名称、标签集合和数值
				// 适用于使用 instant query(即时查询)获取的数据
				// 比如查询 "up" 指标会返回所有监控目标的当前存活状态
				// 或者查询 "node_cpu_seconds_total" 会返回所有节点当前的 CPU 使用情况
...
func main() {
	...
	// 初始化Prometheus客户端
	client, err := initPrometheusClient(config.PrometheusURL) // 初始化Prometheus客户端
	if err != nil {
